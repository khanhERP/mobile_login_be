...
export async function tenantMiddleware(
  ...
  try {
    ...
    console.log("🔍 Headers:", req.session.tenantDomain);

    // Read X-Tenant-Domain header from client (sent from localStorage)
    const tenantDomainHeader = req.headers["x-tenant-domain"] as string;

    // If header exists and session doesn't have tenantDomain, save it to session
    if (tenantDomainHeader && !req.session.tenantDomain) {
      req.session.tenantDomain = tenantDomainHeader;
      ...
      await new Promise<void>((resolve, reject) => {
        req.session.save((err) => {
          if (err) {
            ...
            console.log(
              "💾 Saved tenantDomain to session from header:",
              tenantDomainHeader,
            );
            resolve();
          }
    ...
    // Priority 1: Check session for tenant info (set during login or from header)
    let tenantDomain = req.session?.tenantDomain ?? null;
    // For development, allow tenant to be specified via query parameter
    let subdomain = req.query.tenant as string;

    // Use tenantDomain from session if available, otherwise use header, fallback to origin
    let origin = tenantDomain || tenantDomainHeader || req.headers.origin;

    // Extract store code from domain if applicable
    let storeCode = tenantManager.extractStoreCode(origin);
    console.log("storeCode", storeCode);
    console.log("origin", origin);
    console.log("tenantDomain from session", tenantDomain);
...
export async function getTenantDatabase(req: TenantRequest) {
  console.log("🔍 Getting tenant database connection...", req.tenant);
  if (!req.tenant) {
