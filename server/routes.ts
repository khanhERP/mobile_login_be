...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req: TenantRequest, res) => {
    try {
      ...
      const [user] = await database
        ...
        .where(
          and(
            eq(sysUsers.userName, username),
            eq(sysUsers.password, password),
            eq(sysUsers.isActive, true),
          ),
        )
        .limit(1);
        ...
        await new Promise<void>((resolve, reject) => {
          req.session.save((err) => {
            if (err) {
              ...
              console.log(
                "✅ Session saved successfully with domain:",
                user.domain,
              );
              resolve();
            }
  ...
  app.post("/api/auth/change-password", async (req: TenantRequest, res) => {
    try {
      ...
      const userQuery = username
        ? eq(sysUsers.userName, username)
        : eq(sysUsers.isActive, true); // Fallback if no username provided

      const [user] = await database
        .select()
        .from(sysUsers)
        .where(and(userQuery, eq(sysUsers.isActive, true)))
        .limit(1);
      ...
      // Update password in sys_user table
      await database
        .update(sysUsers)
        .set({
          password: newPassword,
        })
        .where(eq(sysUsers.id, user.id));
    ...
    async (req: TenantRequest, res) => {
      try {
        ...
        let ordersQuery = await database
          .select({
            ...orders,
            items: sql`json_agg(${orderItemsTable}.* ORDER BY ${orderItemsTable.id})`,
          })
          .from(orders)
          .leftJoin(orderItemsTable, eq(orders.id, orderItemsTable.orderId))
          .where(
            or(
