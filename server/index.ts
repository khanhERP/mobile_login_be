...
app.use(
  session({
    secret:
      process.env.SESSION_SECRET || "edpos-secret-key-change-in-production",
    resave: false,
    saveUninitialized: true, // CHANGED: Create session even if not modified
    name: "edpos.sid", // Custom cookie name
    cookie: {
    ...
    proxy: true, // ADDED: Trust proxy for secure cookies
  }),
);
...
app.use((req, res, next) => {
  ...
  const allowedOrigins = [
    ...
    "https://mobile.edpos.vn",
    "https://0318671828.edpos.vn",
    "https://hazkitchen-mobile.edpos.vn",
    "https://0318225421-mobile.edpos.vn",
  ...
  res.header(
    ...
    "Origin, X-Requested-With, Content-Type, Accept, Authorization, x-tenant-id, X-Tenant-Domain",
  );
  res.header(
  ...
  if (origin) {
    ...
    const isMobileEdpos = origin.includes("mobile.edpos.vn") || origin.includes("edpos.vn");

    // Always return the specific origin when credentials are present
    res.header("Access-Control-Allow-Origin", origin);
    res.header("Access-Control-Allow-Credentials", "true");
    
    if (isAllowedOrigin || isReplitDev || isVercelApp || isMobileEdpos) {
      console.log(
        `✅ CORS allowed for origin: ${origin} (allowed: ${isAllowedOrigin}, replit: ${isReplitDev}, vercel: ${isVercelApp}, mobile: ${isMobileEdpos})`,
      );
    } else {
      console.log(
    ...
    // No origin (same-origin requests) - only use wildcard when no credentials
